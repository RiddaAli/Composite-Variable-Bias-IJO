...
list_of_packages <- c("foreign", "summarytools", "data.table","fastDummies", 
                      "ggplot2", "Rfast", "dplyr", "ggpubr", "tidyverse", "haven")
...
} else { devtools::install_github('jtextor/dagitty/r') }
...
makeFldrPlots <- "Plots"
makeFldrData <- "Data"
if (!dir.exists(Plots)) dir.create(file.path(getwd(), makeFldrPlots))
if (!dir.exists(Data)) dir.create(file.path(getwd(), makeFldrData))
...
# Moving the sav data files to the "Data" folder
sav_dataFiles <- c("ncds4.sav", "ncds5cmi.sav", "selected_data_ncd_age23_imputed.csv") 
move_files <- function(f){
  file.rename( from = file.path(getwd(), f) ,
               to = file.path(getwd(),"/Data/", f) )
}

lapply(sav_dataFiles, move_files)

ncds4                 <- tibble(read_sav(file = paste0(Data, "ncds4.sav"))) 
...
################################################################################
## MAIN PROGRAM - MAKE MANUAL CHANGES TO THE cvs FILE FOR IMPUTED VALUES -------

# In the file 'selected_data_ncd_age23.csv' there are missing values for baseline height (BaseHt). 
# The missing values can be filled manually by using the follow up height (FUpHt) variable from the NCDS data at age 33 
# (assuming height remains unchanged between ages 23 and 33)
# saved as 'selected_data_ncd_age23_imputed.csv'.
# Switch the lines used below to read in the imputed data instead of the original data.
# This will then reproduce exactly what was done for the article, otherwise there 
# will be small differences due to the missing values.

NCD_Age23             <- tibble(read.csv(file = paste0(Data, "selected_data_ncd_age23.csv"), header = TRUE, sep = ","))
...
ncds_age33            <- tibble(read_sav(file = paste0(Data, "ncds5cmi.sav")))
...
write.csv(merged_df, file = paste0(Data, "merged_df.csv"), row.names = FALSE)
...
suppressMessages(stview(dfSummary(tmp), file = paste0(Plots, "Final_df.html"), footnote = NA))
